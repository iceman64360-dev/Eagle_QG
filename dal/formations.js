...
function get(id) {
  return new Promise((resolve, reject) => {
    db.get('SELECT * FROM formations WHERE id = ?', [id], (err, row) => {
...
function create(data) {
  ...
  const stmt = db.prepare(`INSERT INTO formations (id, nom, type, dateDebut, dateFin, statut, formateur, participants, objectifs, notes, confirmed, createdAt, updatedAt)
                           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`);
...
function update(id, updates) {
  return get(id).then(current => {
    ...
    const stmt = db.prepare(`UPDATE formations SET nom=?, type=?, dateDebut=?, dateFin=?, statut=?, formateur=?, participants=?, objectifs=?, notes=?, confirmed=?, createdAt=?, updatedAt=? WHERE id=?`);
    return new Promise((resolve, reject) => {
      stmt.run([
        data.nom,
        data.type,
